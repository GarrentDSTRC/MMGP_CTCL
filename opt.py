...
def optIGD(mymodel,mylikelihood,num_task=-2,testmode="test_WFG",train_x=[]):
    ...
    df = pd.DataFrame(np.hstack([X, F]), columns=["st",	"ad","theta","phi","m","p","t", "ct", "eta"])
...
class MyProblem(Problem):
    ...
    def __init__(self,num_task=-2,testmode=[],constr=[]):
