...
if __name__=="__main__":
    ...
    for j in range(Episode):
        ...
        #f.close()
        ##########################################################infill###################
        X,Y=infillGA(model, likelihood, Infillpoints, dict, num_tasks,"EI", device=device, cofactor=cofactor, y_max=[torch.max(train_y[:,0]).item() ,torch.max(train_y[:,1]).item()], offline=Offline,train_x=train_x,testmode=testmode,final_population_X=pop)
