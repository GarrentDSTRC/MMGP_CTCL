...
#归一化只在这里归一化
    if mode=="experiment":
        ...
        for i in range(int(num_p/8)):
            for j in range(8):
                np.savetxt(r'.\MMGP_OL%d\dataX.txt' % (j % 8), X[i * 8 + j:i * 8 + j + 1, :], delimiter=',', fmt='%.02f,%.02f,%.02f,%.02f,%.02f,%.02f,%.02f')
...
def infillGA(model, likelihood, n_points, dict, num_tasks=1, method="error", cofactor=[0.5,0.5], offline=1, device = torch.device("cpu"),y_max=999,train_x=[],testmode="experiment",final_population_X=[]):
    "num_task<0    ->use the multi-fidelity kernel"
    #num_task=1 Single GP; num_task=0 Raw;num_task=2 Multitask;num_task=-2  and multifidelity ;num_task=-1 multifidelity
    ...
    final_population_individuals = [creator.Individual(x) for x in final_population_X]
    ...
    for individual in final_population_individuals:
        ...
        individual.fitness.values = individual.fitness.values = evaluateEI(individual,
                                               model=model,
                                               ...
                                               object=2)
    # Get Pareto fronts
    pareto_front_ALL = tools.emo.sortLogNondominated(final_population_individuals, len(final_population_individuals), first_front_only=False)
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        ...
        for pareto_front in pareto_front_ALL:
            for ind in pareto_front:
                ...
                candidate = [round(x, 2)  for i, x in enumerate(ind)]
                if candidate not in candidates and candidate not in train_x.tolist()  :
        ...
        X,Y=findpointOL(X,num_task=num_tasks,mode=testmode)
        return X,Y
...
def evaluateEISO(individual,model,likelihood,y_max,cofactor):
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        for i in range(len(STP)):
            ind[i]=individual[i]*(UPB[i]-LOWB[i])/(STP[i]-1)+LOWB[i]
        ind=torch.tensor(ind).to(device).to(torch.float32).unsqueeze(0)
        A=likelihood(*model( ind,  ind))
